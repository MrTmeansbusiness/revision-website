  ...
  function renderBuilder(){
    mount(pageShell(`
      <section class="bg-white border rounded-xl p-4 md:p-6 shadow-sm">
        <h2 class="font-semibold mb-3">Manual entry</h2>
        <form id="manualForm" class="space-y-4">
          <div class="grid md:grid-cols-2 gap-3">
            ...
            <label class="text-sm">Topic (optional)
      ...
      <section class="mt-6 bg-white border rounded-xl p-4 md:p-6 shadow-sm">
        ...
        <div class="grid md:grid-cols-2 gap-3">
          <label class="text-sm">GitHub owner
            <input id="ghOwner" class="border rounded px-2 py-1 w-full" placeholder="e.g. mrtmeansbusiness">
          </label>
    ...
    // No file helpers needed in minimal mode
    ...
    (function prefillGh(){
      const cfg = loadGhCfg();
      if (ghOwnerEl) ghOwnerEl.value = cfg.owner || ghOwnerEl.value || 'mrtmeansbusiness';
      if (ghRepoEl) ghRepoEl.value = cfg.repo || ghRepoEl.value || 'revision-website';
    ...
    document.getElementById('mAdd')?.addEventListener('click', async ()=>{
      const section = (mSection?.value||'s2');
      const topic = (mTopic?.value||'general').trim()||'general';
      const text = (mText?.value||'').trim();
      const a = (mA?.value||'').trim();
      const b = (mB?.value||'').trim();
      const c = (mC?.value||'').trim();
      const d = (mD?.value||'').trim();
      const answer = parseInt(mAns?.value||'0',10) || 0;
      if (!text || !a || !b || !c || !d) { alert('Please enter the question and all four options.'); return; }
      const bank = loadCustomBank();

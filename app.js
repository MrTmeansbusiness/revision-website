    ...
    manualForm?.addEventListener('submit', async (e)=>{
      ...
      const choices = [mA.value, mB.value, mC.value, mD.value].map(s=>String(s||'').trim());
      const answer = parseInt(mAns.value,10)||0;
      ...
      const payload = { topic, text, choices, answer };
    ...
    mount(pageShell(`
      <section class="bg-white border rounded-xl p-4 md:p-6 shadow-sm">
        ...
        <form id="manualForm" class="space-y-4">
          ...
          <div>
            <button class="bg-emerald-600 text-white rounded-md px-4 py-2 font-semibold hover:bg-emerald-700">Add to bank</button>
          </div>
        </form>
      </section>
      <section class="bg-white border rounded-xl p-4 md:p-6 shadow-sm">

  ...
  function renderBuilder(){
    mount(pageShell(`
      <section class="bg-white border rounded-xl p-4 md:p-6 shadow-sm">
        <h2 class="font-semibold mb-3">1) Manual entry</h2>
        <form id="manualForm" class="space-y-4">
          <div class="grid md:grid-cols-2 gap-3">
            <label class="text-sm">Section
              <select id="mSection" class="border rounded px-2 py-1 w-full">
            ...
            <label class="text-sm">Topic
              <input id="mTopic" class="border rounded px-2 py-1 w-full" placeholder="e.g. demand, elasticity, general">
            </label>
          </div>
          <label class="text-sm block">Question
            <textarea id="mText" class="w-full border rounded p-2 text-sm" rows="3" placeholder="Enter the question text"></textarea>
          </label>
          <div>
            <div class="text-sm mb-1">Options (A–D)</div>
            <div class="grid md:grid-cols-2 gap-2">
              <input id="mA" class="border rounded px-2 py-1" placeholder="Option A">
              <input id="mB" class="border rounded px-2 py-1" placeholder="Option B">
              <input id="mC" class="border rounded px-2 py-1" placeholder="Option C">
              <input id="mD" class="border rounded px-2 py-1" placeholder="Option D">
            </div>
          </div>
          <div class="grid md:grid-cols-2 gap-3 items-center">
            <label class="text-sm">Correct answer
              <select id="mAns" class="border rounded px-2 py-1">
            ...
            <div class="text-sm">
              <label>Image (optional)
                <input id="mImg" type="file" accept="image/*" class="block">
              </label>
            </div>
          </div>
          <div>
            <button id="mAdd" type="button" class="bg-emerald-600 text-white rounded-md px-4 py-2 font-semibold hover:bg-emerald-700">Add to bank</button>
          </div>
        </form>
      </section>
      <section class="bg-white border rounded-xl p-4 md:p-6 shadow-sm">
        <h2 class="font-semibold mb-3">2) Upload PDF</h2>
      ...
      <section class="mt-6 bg-white border rounded-xl p-4 md:p-6 shadowsm">
        <h2 class="font-semibold mb-3">3) Import JSON (optional)</h2>
    ...
    function publishedHashes(published){
      const set = new Set();
      for (const sec of Object.keys(published||{})){
        (published[sec]?.questions||[]).forEach(q=>{ set.add(hashQ(q)); });
      }
      return set;
    }
    function renderPerItemList(){
      const wrap = document.getElementById('perItemList');
      if (!wrap) return;
      wrap.innerHTML = '';
      const custom = loadCustomBank();
      const published = loadPublishedBank();
      const pubSet = publishedHashes(published);
      const frag = document.createDocumentFragment();
      const sections = Object.keys(custom||{}).sort();
      if (sections.length===0){
        const empty = document.createElement('div');
        empty.className = 'text-sm text-slate-500';
        empty.textContent = 'No custom questions yet.';
        wrap.appendChild(empty);
        return;
      }
      sections.forEach(sec => {
        const secBlock = custom[sec] || { questions: [] };
        (secBlock.questions||[]).forEach((q, idx) => {
          const isPub = pubSet.has(hashQ(q));
          const card = document.createElement('div');
          card.className = 'border rounded-xl p-3 bg-white flex items-start gap-3';
          card.innerHTML = `
            <div class="flex-1">
              <div class="text-sm font-medium">[${sec}] ${q.topic || 'general'}</div>
              <div class="text-sm text-slate-700 mt-1 line-clamp-3">${escapeHtml(q.text||'')}</div>
              <div class="text-xs text-slate-500 mt-1">Choices: ${q.choices?.map(escapeHtml).join(' | ')}</div>
            </div>
            <div class="shrink-0 grid gap-2 justify-items-end">
              <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs ${isPub?'bg-emerald-100 text-emerald-800 border border-emerald-200':'bg-amber-100 text-amber-800 border border-amber-200'}">${isPub?'published':'not published'}</span>
              <button data-sec="${sec}" data-idx="${idx}" class="pubOne ${isPub?'opacity-50 cursor-not-allowed':''} rounded-md px-3 py-1.5 text-sm border">Publish</button>
            </div>`;
          frag.appendChild(card);
        });
      });
      wrap.appendChild(frag);
      wrap.querySelectorAll('button.pubOne').forEach(btn => {
        btn.addEventListener('click', async () => {
          if (btn.classList.contains('cursor-not-allowed')) return;
          const sec = btn.getAttribute('data-sec');
          const idx = parseInt(btn.getAttribute('data-idx'),10);
          await publishOne(sec, idx, btn);
        });
      });
    }
    async function publishOne(sec, idx, btn){
      try {
        const owner = (ghOwnerEl?.value||'').trim();
        const repo = (ghRepoEl?.value||'').trim();
        const branch = (ghBranchEl?.value||'main').trim()||'main';
        const path = (ghPathEl?.value||'bank_template.json').trim();
        const token = (ghTokenEl?.value||'').trim();
        if (!owner || !repo || !path || !token) { alert('Fill GitHub owner/repo/branch/path/token first.'); return; }
        const custom = loadCustomBank();
        const q = custom[sec]?.questions?.[idx];
        if (!q) return;
        const published = loadPublishedBank();
        const next = JSON.parse(JSON.stringify(published||{}));
        if (!next[sec]) next[sec] = { topics: [], questions: [] };
        const tset = new Set((next[sec].topics||[]).map(t=>t.id));
        if (q.topic && !tset.has(q.topic)) next[sec].topics.push({ id: q.topic, name: q.topic });
        const seen = new Set((next[sec].questions||[]).map(x=>hashQ(x)));
        const h = hashQ(q);
        if (!seen.has(h)) next[sec].questions.push(q);
        const json = JSON.stringify(next, null, 2);
        const base64 = toBase64Utf8(json);
        btn.disabled = true; btn.textContent = 'Publishing…';
        const sha = await githubGetFileSha(owner, repo, path, branch, token).catch(()=>null);
        await githubPutFile(owner, repo, path, branch, token, base64, sha||undefined);
        savePublishedBank(next);
    ...
    document.getElementById('publishAllMissing')?.addEventListener('click', async ()=>{
      const list = Array.from(document.querySelectorAll('#perItemList button.pubOne'));
      for (const btn of list){
        if (btn.classList.contains('cursor-not-allowed')) continue;
        await publishOne(btn.getAttribute('data-sec'), parseInt(btn.getAttribute('data-idx'),10), btn);
      }
    });

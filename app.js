...
route('/section', () => {
  ...
  const topics = getBank()[section]?.topics || [];
...
// Merge default QUESTION_BANK with custom, filtering out draft questions
function getBank(){
  // Merge default QUESTION_BANK with custom additions
  const custom = loadCustomBank() || {};
  const defaults = (typeof QUESTION_BANK !== 'undefined' ? QUESTION_BANK : {});
  const allSections = new Set(['s1','s2','s3','s4','s5','s6', ...Object.keys(defaults), ...Object.keys(custom)]);
  const out = {};
  allSections.forEach(sec => {
    const d = defaults[sec] || { topics: [], questions: [] };
    const c = custom[sec] || { topics: [], questions: [] };
    // merge topics by id
    const tMap = new Map();
    (d.topics||[]).forEach(t=>{ if(t&&t.id) tMap.set(t.id, t); });
    (c.topics||[]).forEach(t=>{ if(t&&t.id && !tMap.has(t.id)) tMap.set(t.id, t); });
    const topics = Array.from(tMap.values());
    const questions = [ ...(d.questions||[]), ...((c.questions||[]).filter(q=>!q.draft)) ];

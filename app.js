    ...
    <div class="min-h-[70vh] grid place-items-center relative">
      <div class="w-full max-w-md mx-auto">
        ...
        <div class="text-center mt-4 text-sm">
          ${isBankEmpty() ? `<a href="#/import" class="text-blue-700 hover:underline">Have a class bank file? Import it</a>` : ''}
        </div>
      </div>
    </div>
  `));
      ...
      <div class="space-y-3">
        ...
        <div class="bg-white p-4 rounded-xl border shadow-sm">
          ...
          <div class="flex gap-3 flex-wrap">
            <a href="#/import" class="rounded-md px-4 py-2 border">Import bank</a>
          </div>
...
// Import page (student-facing): load a teacher-provided bank.json into localStorage
route('/import', () => {
  mount(pageShell(`
    <div class="max-w-xl mx-auto">
      <h2 class="text-xl font-semibold mb-2">Import Question Bank</h2>
      <p class="text-slate-600 mb-4">Upload the bank JSON file provided by your teacher. This saves questions to your browser only.</p>
      <div class="bg-white p-5 rounded-xl border shadow-sm space-y-3">
        <input id="bankFile" type="file" accept="application/json" class="block">
        <textarea id="bankText" class="w-full border rounded p-2 text-sm" rows="6" placeholder='Or paste JSON here'></textarea>
        <div class="flex gap-3">
          <button id="importBtn" class="bg-blue-600 text-white rounded-md px-4 py-2 font-semibold hover:bg-blue-700">Import</button>
          <button id="cancelBtn" class="rounded-md px-4 py-2 border">Cancel</button>
        </div>
        <p class="text-xs text-slate-500">Note: This does not download any copyrighted material from this site. It only stores the file you provide in your own browser.</p>
      </div>
    </div>
  `));
  const file = document.getElementById('bankFile');
  const text = document.getElementById('bankText');
  ...
  document.getElementById('importBtn').addEventListener('click', async ()=>{
    try {
      ...
      if (file.files && file.files[0]){
        ...
        payload = normalizeImportedBank(JSON.parse(txt));
      } else if (text.value.trim()){
        payload = normalizeImportedBank(JSON.parse(text.value.trim()));
      } else { alert('Choose a JSON file or paste JSON.'); return; }

      // Merge into existing custom bank
      mergeIntoCustomBank(payload);
      alert('Bank imported. You can now take quizzes.');
      navigate('/');
    } catch (e) {
...
// Use ONLY the custom bank (non-draft). No bundled defaults.
function getBank(){
  const custom = loadCustomBank() || {};
  const allSections = new Set(['s1','s2','s3','s4','s5','s6', ...Object.keys(custom)]);
  const out = {};
  allSections.forEach(sec => {
    const c = custom[sec] || { topics: [], questions: [] };
    const topics = Array.from((c.topics||[]));
    const questions = (c.questions||[]).filter(q=>!q.draft);
    out[sec] = { topics, questions };
  });
  return out;
}
...
route('/bank', () => {
  ...
  function renderBuilder(){
    ...
    clearBtn.addEventListener('click', ()=>{ if (confirm('Clear all custom questions?')){ saveCustomBank({}); alert('Cleared.'); }});
  }
